...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Ambil info client..."));
        const service = new SpeedTestService();
        await service.fetchClientInfo();

        await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Cari server terbaik..."));
        const bestServer = await service.findBestServer();

        await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Uji latency..."));
        const latencySpeed = (await service.testLatency(bestServer, 5)).latency;

        await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Uji download..."));
        const downloadSpeed = await service.testDownload(bestServer, null, {
            threads: 4,
            duration: 10000
        });

        await ctx.editMessage(ctx.id, speedtestMsg.key, ctx.format.info("Uji upload..."));
